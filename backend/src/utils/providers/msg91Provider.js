...
class Msg91Provider {
  ...
  /**
   * Send custom OTP using MSG91 v5 SendOTP API
   * POST https://control.msg91.com/api/v5/otp
   */
  async sendOTP(toPhone, otp) {
    if (!toPhone) return { success: false, error: 'Phone number is required' };

    // MSG91 expects number with country code but NO leading '+'
    ...
    if (!this.enabled) {
      console.log('\n💬 ====== [TEST MODE MSG91 OTP PREVIEW] ======');
      console.log(`To:      ${formattedPhone}`);
      console.log(`OTP:     ${otp}`);
      console.log(`Template:${this.templateId}`);
      console.log('==============================================\n');
      return { success: true, requestId: `TEST-MSG91-OTP-${Date.now()}` };
    }

    const maxRetries = 3;
    let attempt = 0;

    const url = `https://control.msg91.com/api/v5/otp?template_id=${this.templateId}&mobile=${formattedPhone}&authkey=${this.authKey}`;
    ...
    while (attempt <= maxRetries) {
      ...
      try {
        ...
        // MSG91 response status is usually in body type: { type: "success" / "error" } or standard HTTP status
        if (!response.ok || (responseData && responseData.type === 'error')) {
          const errMsg = responseData.message || response.statusText || 'Unknown error';
          console.error(`❌ MSG91 OTP API error (Attempt ${attempt + 1}/${maxRetries + 1}):`, errMsg);
          throw new Error(errMsg);
        }

        console.log(`✅ OTP delivered successfully via MSG91 (Request ID: ${responseData.request_id || 'N/A'})`);
        return { success: true, requestId: responseData.request_id || 'N/A' };
      } catch (error) {
        clearTimeout(timeoutId);
        attempt++;
        const isTimeout = error.name === 'AbortError';
        console.warn(`⚠️ MSG91 OTP attempt ${attempt} failed: ${isTimeout ? 'Request Timed Out (5s)' : error.message}.`);
        ...
        if (attempt <= maxRetries) {
          ...
          console.error(`❌ MSG91 OTP dispatch failed after ${maxRetries + 1} attempts for: ${formattedPhone}`);
