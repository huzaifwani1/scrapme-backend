...
(() => {
  ...
  function startEventSource() {
    ...
    const sseUrl = window.location.origin + '/api/operations/events';
  ...
  function showJobDetails(order) {
    ...
    if (isReadOnly) {
      ...
      // Update OTP interface based on whether OTP is enabled
      const enableOtp = order.enableOtp !== false;

      if (!enableOtp) {
        elRemoveClass('#otp-bypass-block', 'hidden');
        elAddClass('#otp-verification-workflow', 'hidden');
        elRemoveClass('#direct-pickup-workflow', 'hidden');
        elAddClass('#otp-status-block', 'hidden');
        elAddClass('#otp-dev-banner', 'hidden');
      } else {
        elAddClass('#otp-bypass-block', 'hidden');
        elRemoveClass('#otp-verification-workflow', 'hidden');
        elAddClass('#direct-pickup-workflow', 'hidden');

        if (order.otp) {
          elRemoveClass('#otp-status-block', 'hidden');
          const phoneVal = req.phone || '—';
          elSetText('#otp-masked-phone', maskPhoneNumber(phoneVal));
          elSetText('#otp-status-badge', 'Pending Verification');
          elAddClass('#btn-generate-otp', 'hidden');
          elRemoveClass('#btn-resend-otp', 'hidden');

          if (order._test_otp) {
            elRemoveClass('#otp-dev-banner', 'hidden');
            elSetText('#otp-dev-code', order._test_otp);
          } else {
            elAddClass('#otp-dev-banner', 'hidden');
          }
        } else {
          // If not generated and timer is not active, reset buttons
          if (!resendCooldownInterval) {
            elAddClass('#otp-status-block', 'hidden');
            elRemoveClass('#btn-generate-otp', 'hidden');
            elAddClass('#btn-resend-otp', 'hidden');
          }
          elAddClass('#otp-dev-banner', 'hidden');
        }
      }
  ...
  function maskPhoneNumber(phone) {
    ...
    if (digits.length > 5) {
      return digits.substring(0, digits.length - 5) + 'XXXXX';
    }
    return phone.substring(0, Math.max(0, phone.length - 5)) + 'XXXXX';
  }

  function startResendCooldown() {
    const resendBtn = $('#btn-resend-otp');
    const generateBtn = $('#btn-generate-otp');
  ...
  function resetOtpVerificationUI() {
    if (resendCooldownInterval) {
      clearInterval(resendCooldownInterval);
      resendCooldownInterval = null;
    }
    const statusBlock = $('#otp-status-block');
    ...
    const verificationInput = $('#verification-code');
    if (verificationInput) verificationInput.value = '';
  }

  window.triggerOtpGenerate = async () => {
    if (!state.selectedOrderId) return;
    try {
      const res = await apiFetch(`/orders/${state.selectedOrderId}/otp/generate`, {
        method: 'POST',
        body: JSON.stringify({
          latitude: state.simLat,
          longitude: state.simLng
      ...
      showToast('OTP has been sent to the customer’s registered mobile number.');
      
      // Update UI elements
      const activeOrder = state.activeOrders.find(o => o._id === state.selectedOrderId);
      const phoneVal = (activeOrder && activeOrder.requestId && activeOrder.requestId.phone) || '';
      
      const statusBlock = $('#otp-status-block');
      if (statusBlock) statusBlock.classList.remove('hidden');
      const maskedPhone = $('#otp-masked-phone');
      if (maskedPhone) maskedPhone.textContent = maskPhoneNumber(phoneVal);
      const statusBadge = $('#otp-status-badge');
      if (statusBadge) statusBadge.textContent = 'Pending Verification';

      if (res && res.testOtp) {
        elRemoveClass('#otp-dev-banner', 'hidden');
        elSetText('#otp-dev-code', res.testOtp);
      } else {
        elAddClass('#otp-dev-banner', 'hidden');
      }
  ...
  window.triggerOtpResend = async () => {
    ...
    try {
      ...
      showToast('New OTP has been generated and sent to the customer.');
      
      const codeInput = $('#verification-code');
  ...
  window.verifyOtpAndComplete = async () => {
    const activeOrder = state.activeOrders.find(o => o._id === state.selectedOrderId);
    const enableOtp = activeOrder ? activeOrder.enableOtp !== false : true;

    const otpInput = $('#verification-code');
    let otp = otpInput ? otpInput.value.trim() : '';
    ...
    try {
      ...
      showToast('Order pickup completed successfully! 🎉');
      state.selectedOrderId = null;
      state.extraDevices = [];
      resetOtpVerificationUI();
      loadPartnerOrders();
